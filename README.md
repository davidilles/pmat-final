...
As part of my ongoing efforts to continously learn something new (and hands-on) related to information security, I have recently completed the excellent [Practical Malware Analysis & Triage](https://academy.tcm-sec.com/p/practical-malware-analysis-triage) course offered by [TCM Security](https://academy.tcm-sec.com/) . The conclusion of this course is a final challenge that asks students to choose one of the malware samples covered during the course, write a triage report of the sample, write a set of detection rules (using [Yara](https://github.com/VirusTotal/yara)) for it, and publish the findings. 
This post is my submission to satisfy this course closure requirement.
...
I have chosen WannaCry as the subject of my report for two reasons. First, it is a real-live malware sample which (to me at least) is more interesting to look at compared to lab-grown artificial specimen. Second, the WannaCry attack was famiously halted by [Marcus Hutchins](https://portswigger.net/daily-swig/marcus-hutchins-on-halting-the-wannacry-ransomware-attack-still-to-this-day-it-feels-like-it-was-all-a-weird-dream)  by identifying a "killswitch" in the malware, which story always fascinated me as some sort of "cyber-superhero" deed. 

### The Purpose of The Post

The purpose of this post, apart from satisfying the course requirement :) is to demistify how simple techniques (all covered in the course) can be used to identify WannaCry's killswitch and confirm that the killswitch is indeed real in order to showcase that these simple techniques can be very effective even when applied to real, extremely destructive malware.

This post is **not** intended to provide a full analysis of the malware, most notably its propagation mechanism is completely ignored by the analysis.

## Taking a First Look

Once specimen `Ransomware.wannacry.exe` is received, even before it is first "detonated" (ran) some basic information can be extracted from it.
...
Which could be used to investigate if it was seen in the wild before (for example by searching on [VirusTotal](https://www.virustotal.com/gui/home/search) ). This step is excluded from this post, as WannaCry is a well-known sample at this point.
...
A strange URL can be found potentially suggesting a command and control server or other remote host related to the attacker's infrastructure:
```
http://www.iuqerfsodp9ifjaposdfjhgosurijfaewrwergwea.com
```

Cryptography-related strings can be found suggesting potential ransomware capabilities:
```
...
Analysing the executable using [PEView](http://wjradburn.com/software/) the potentiality of cryptographic capabilites is further supported by inspecting the `Import Address Table`:

![](/_images/Pasted%20image%2020230216224845.png)  

`PEView` also provides evidences that suggests that the specimen does not use packing techniques, but these are excluded for simplicity.
...
For internet simulation, `inetsim` was configured to provide the services: `DNS`, `HTTP`, `SMTP`, `HTTPS`, `POP3`, `FTPS`, `SMTPS`, `POP3S`, `FTP`, and to respond to any query.
...
The malware, when detonated without network connectivity, almost immediately produces significant indicators of compromise. For example.

A pop-up informing the user that their files are encrypted:
...
The malware when detonated with internet simulation can be observed to connect to the URL identified above (`http://www.iuqerfsodp9ifjaposdfjhgosurijfaewrwergwea.com`):

![](/_images/Pasted%20image%2020230217224137.png)  

And (at first glance) does not exhibit any other significant activity.

**This is an important realization. It suggests that the URL acts as a killswitch to the malware, however the apparent lack of other indicators of compromise does not mean that the malware is completely defused. The URL might only instruct the malware to be more stealthy, and further analysis is required to ensure that the suspected killswitch is indeed a killswitch.**

## Investigating the Suspected Killswitch

### Static Analysis

To confirm the killswitch as genuine, the specimen can be loaded into a disassembler (`Cutter` was used for this analysis as it is the tool covered on the course) and the logic related to the URL can be observed. 
...
Also, no other action is taken by the program except system calls related to visiting the url (`InternetOpanA`, `InternetOpenURlA`), which calls are followed by a test on the success of the connection:
...
The behviour can also be confirmed dynamically, by loading the specimen into a debugger (`x32dbg` used as it is the course's preferred debugger) and following its execution.
...
The system calls to `InternetCloseHandle`:
...
Leading to the `exit` from the program:
...
The rule matches any `PE` file that contains the killswitch domain. During testing it could successfully identify the specimen (and only the specimen) on the lab VM:
```
C:\Users\analyst\Desktop
